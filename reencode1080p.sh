...
if [[ "$VIDEO_BITRATE" -lt 800000 && "$VIDEO_WIDTH" -le 1920 && "$VIDEO_HEIGHT" -le 1080 ]]; then
    ...
    if [[ "$VIDEO_WIDTH" -eq 1920 && "$VIDEO_HEIGHT" -eq 1080 ]]; then
        FFMPEG_CMD+=" -map 0:v:0 -c:v libx265 -preset medium -crf 28 -vf scale=1920:1080"
    elif [[ "$VIDEO_WIDTH" -gt 1920 || "$VIDEO_HEIGHT" -gt 1080 ]]; then
        ...
        if [[ "$VIDEO_WIDTH" -gt "$VIDEO_HEIGHT" ]]; then
            # Scale width down to 1920, and calculate the new height to maintain aspect ratio
            VIDEO_WIDTH=1920
            VIDEO_HEIGHT=$(echo "scale=0; $VIDEO_WIDTH / $ASPECT_RATIO" | bc)
        else
            # Scale height down to 1080, and calculate the new width to maintain aspect ratio
            VIDEO_HEIGHT=1080
    ...
    elif [[ "$VIDEO_WIDTH" -lt 1920 || "$VIDEO_HEIGHT" -lt 1080 ]]; then
        FFMPEG_CMD+=" -vf scale=$VIDEO_WIDTH:$VIDEO_HEIGHT"
    fi
fi
